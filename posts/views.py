...
class PostListView(View):
  ...
  def post(self, request, *args, **kwargs):
    ...
    if form.is_valid():
      ...
      instance.save()
      messages.success(request, 'Пост был добавлен!')
      return redirect('posts:posts-list')
    messages.error(request, 'Oop! Введите действительные данные!')
...
class PostDeleteView(LoginRequiredMixin, DeleteView):
  ...
  def get_success_url(self, *args, **kwargs):
    messages.success(self.request, 'Сообщение было удалено!')
    return reverse('posts:posts-list')
